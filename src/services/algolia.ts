...
class AlgoliaSearchService extends AbstractSearchService {
	...
	constructor(
		...
		if (!adminApiKey) {
			throw new Error(
				"Please provide a valid Admin Api Key"
			);
		}

		this.client_ = Algolia(
			applicationId,
			adminApiKey
		);
	}
	...
	async addDocuments(
		...
		const transformedDocuments =
			await this.getTransformedDocuments(
				type,
				filteredDocuments
			);

		console.log(
			`Adding ${transformedDocuments.length} published documents to Algolia's ${indexName} index`
		);
		return await this.client_
			.initIndex(indexName)
			.saveObjects(
	...
	async replaceDocuments(
		...
		console.log(
			`Replacing ${transformedDocuments.length} published documents to Algolia's ${indexName} index`
		);
		return await this.client_
			.initIndex(indexName)
			.replaceAllObjects(
	...
	async search(
		...
		if (
			"limit" in paginationOptions &&
			paginationOptions.limit != null
		) {
	...
	async getTransformedDocuments(
		...
		switch (type) {
			case indexTypes.PRODUCTS:
				...
				const errors =
					transformed.filter(rejected);
				if (errors.length) {
					console.error(
						"Error transforming products",
						errors
					);
				}
